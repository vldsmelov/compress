...
def parse_spec_from_sections(sections_data: dict) -> dict:
    ...
    If a suitable table is not found, return an empty spec with a warning instead of
    raising. This allows the downstream pipeline to respond quickly (and unblock the
    aggregator) even when slices do not contain the expected economic section.
    """
    ...
    if not table_part:
        return {
            ...
            "total": 0.0,
            "vat": 0,
            "warning": "Таблица с товарами не найдена в части документа",
        }
        ...
        if "TABLE:" in line and "|" in line:
            ...
            if (
                ...
                try:
                    ...
                    item = {
                        "name": name,
                        "qty": int(qty) if isinstance(qty, float) and qty.is_integer() else qty,
                        "unit": unit,
                        "price": price,
                        "amount": amount,
                        "country": country,

...
Compose запускает локальный Ollama, а сервисы подключаются к нему по адресу `http://ollama:11434` внутри общей сети. Используйте переменные окружения в `docker-compose.yml`, чтобы выбрать нужную модель или адаптировать таймауты. По умолчанию все сервисы ожидают модель `qwen3:14b-8k` на сервисе `ollama`.

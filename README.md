...
- Шлюз сам создаёт reply-очередь и дожидается финального ответа агрегатора.
...
2. **Нарезка**: `document_slicer` забирает задачу, режет документ на 17 частей и отправляет сообщения в очереди `ai_legal_parts`, `ai_econom_parts`, `contract_extractor_parts`. Одновременно публикует в `aggregation_tasks` ожидания по сервисам.
3. **Обработка частей**:
   - `ai_legal` читает все части из своей очереди и отправляет результаты в `aggregation_results`.
   - `ai_econom` обрабатывает `part_16`, публикует ответы в `aggregation_results` и передаёт поле `seller` в очередь `sb_queue`.
   - `ai_sb` получает `seller` из `sb_queue` и отправляет итог в `aggregation_results`.
   - `contract_extractor` извлекает структуру по каждой части и тоже пишет в `aggregation_results`.
4. **Агрегация**: `aggregator` ждёт все ожидаемые ответы для `task_id` из очереди `aggregation_results`, собирает единый JSON и публикует его в reply-очередь, указанную шлюзом.
